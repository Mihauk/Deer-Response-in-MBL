...
def spin(A, i, N):
	...
	assert (A=="X" or A=="Y" or A=="Z" or A=="I"), "Specify the correct spin matrices from X, Y, Z and I"
	s=1
	if A=='X':
		sp=s_x
	elif A=='Y':
...
def h_rfxxz( J, J_prime, N, h_v):
	H=0
	for i in range(N):
		H = H + J*(np.dot(spin("X",i,N),spin("X",(i+1)%N,N)) + np.dot(spin("Y",i,N),spin("Y",(i+1)%N,N))) + J_prime*(np.dot(spin("Z",i,N),spin("Z",(i+1)%N,N))) + h_v[i]*spin("Z",i,N)
	return (H)

def h_rfxxz_obc( J, J_prime, N, h_v):
	H=0
	for i in range(N-1):
		H = H + J*(np.dot(spin("X",i,N),spin("X",(i+1),N)) + np.dot(spin("Y",i,N),spin("Y",(i+1),N))) + J_prime*(np.dot(spin("Z",i,N),spin("Z",(i+1),N))) + h_v[i]*spin("Z",i,N)
	H = H + h_v[N-1]*spin("Z",N-1,N)
	return (H)
