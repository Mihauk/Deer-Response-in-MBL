	...
	for k in range(dis_sam):
		...
		for j in range(samples):
			...
			for i in range(Ngrid):
				...
				D[k, j, m, i] = cm.check_real(np.dot(chi_t_dagg, np.dot(sigma_z_1, np.dot(sigma_z_2,chi_t))) - (np.dot(chi_t_dagg, np.dot(sigma_z_1, chi_t))*np.dot(chi_t_dagg, np.dot(sigma_z_2, chi_t))))

with open('/home/abhishek/Documents/ny project/data/hv10_N'+str(N)+'_eav50_ovr_egns_dis100_t100000_d_rel_tsi_s14_Ngrid200_spa_corr.pickle', 'wb') as data:
